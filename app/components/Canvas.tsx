...
function ProjectBody({ p }: { p: Project }) {
  return (
    <div className="flex flex-col">
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-[#ebe3d2]">
        <Image
          src={p.cover}
          alt={`${p.name} preview`}
          fill
        ...
        />
        {p.badge && (
          <span className="absolute right-2.5 top-2.5 rounded-full bg-[#161310]/90 px-2.5 py-1 text-[9px] font-medium uppercase tracking-[0.15em] text-[#efe6d4] backdrop-blur">
            {p.badge}
          </span>
        )}
      </div>
...
function ContactBar() {
  ...
  return (
    <div className="relative z-[60] mx-auto mt-10 w-full max-w-[1340px] overflow-hidden rounded-2xl bg-[#161310] px-6 py-6 md:px-8">
...
function TopBar() {
  return (
    <header className="mx-auto flex w-full max-w-[1340px] flex-wrap items-center justify-between gap-3 px-1">
      <div>
        <h1 className="text-xl font-normal tracking-tight text-[#26211b] md:text-2xl">María Sebares</h1>
        <span className="text-[10px] font-medium uppercase tracking-[0.22em] text-[#9a7a4c]">
          AI Engineer · Founder of Tumai
        </span>
      </div>
  ...
  return (
    <div className="relative min-h-[100dvh] w-full overflow-x-hidden bg-[#f0e9da] px-4 py-6 text-[#26211b] md:px-8 md:py-8">
      ...
      <div className="mx-auto mt-3 hidden w-full max-w-[1340px] items-center gap-1.5 px-1 text-[10px] uppercase tracking-[0.18em] text-[#9a7a4c]/80 lg:flex">
        <Hand size={12} /> Drag the windows · click a project to open it
      </div>

      {/* Desktop: scattered draggable canvas */}
      <motion.div
        variants={containerVariants}
        initial="hidden"
        animate="visible"
        className="relative mx-auto mt-4 hidden lg:block"
        ...
        <div ref={stageRef} className="absolute inset-0">
          {windows.map((w) => (
            <WindowCard
              ...
              rotate={w.rotate}
              testId={`window-${w.id}`}
              style={{ position: "absolute", left: w.x, top: w.y, width: w.width }}
            >
              {w.body}
            </WindowCard>
          ))}
